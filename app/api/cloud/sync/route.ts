...
export async function POST(request: Request) {
  try {
    ...
    if (!subData || (subData.status !== 'active' && subData.status !== 'trialing')) {
      return NextResponse.json({ error: 'Requires active premium subscription' }, { status: 403 });
    }
    ...
    return NextResponse.json({ error: error.message || 'Failed to sync accounts.' }, { status: 500 });
  }
}
