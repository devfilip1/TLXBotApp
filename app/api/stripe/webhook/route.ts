...
async function ensureProfile(userId: string) {
  ...
  if (error || !user) {
    console.error(
      "[webhook] ensureProfile: usuário não encontrado no Auth:",
      userId,
      error,
    );
...
async function upsertSubscription(
  ...
  if (!periodEnd) {
    ...
    throw new Error(
      "current_period_end ausente — verifique a versão da API do Stripe",
    );
  }
  ...
  console.log(
    `[webhook] subscription ${subscription.id} → user ${userId} (${subscription.status})`,
  );
}
  ...
  try {
    switch (event.type) {
      ...
      case "invoice.payment_failed": {
        ...
        if (!userId) {
          console.warn(
            "[webhook] invoice event: could not resolve user for customer",
            invoice.customer,
          );
          break;
        }
        ...
        // Sync subscription status on invoice success or failure.
        const subscriptionId =
