...
export function useHomeViewModel() {
  const { user, session, planInfo, isLoading: isAuthLoading } = useAuthViewModel();
  const { accounts, isLoadingAccounts, addAccount, editAccount, deleteAccount } = useAccountsViewModel(user?.id);

  const [isGenerating, setIsGenerating] = useState(false);
  const [accountErrorStatus, setAccountErrorStatus] = useState<Record<string, boolean>>({});
  ...
  const handleSyncToCloud = async () => {
    ...
    try {
      ...
      if (res.ok) {
        alert('Contas sincronizadas com a nuvem com sucesso! A automação rodará automaticamente no próximo ciclo.');
        await fetchAccountStatus();
      } else {
        const errData = await res.json();
        alert(errData.detail || errData.error || 'Falha ao sincronizar contas.');
      }
    } catch (err) {
      console.error(err);
